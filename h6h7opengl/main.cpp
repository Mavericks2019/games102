...
namespace UIUtils {
    ...
    QWidget* createParameterizationControlPanel(GLWidget* glWidget, QWidget* paramTab) {
        ...
        // ==== 添加CVT控件组 ====
        QGroupBox *cvtGroup = new QGroupBox("CVT Iteration");
        QFormLayout *cvtLayout = new QFormLayout(cvtGroup);
        
        QSpinBox *cvtIterSpinBox = new QSpinBox;
        cvtIterSpinBox->setRange(1, 100);
        cvtIterSpinBox->setValue(10);
        QObject::connect(cvtIterSpinBox, QOverload<int>::of(&QSpinBox::valueChanged),
                        [glWidget](int value) { glWidget->cvtIterations = value; });
        
        QLabel *levelLabel = new QLabel("Current Level: 0");
        levelLabel->setStyleSheet("color: white; font-weight: bold;");
        
        QPushButton *cvtButton = new QPushButton("Perform CVT Iteration");
        cvtButton->setStyleSheet(
            "QPushButton {"
            "   background-color: #505050;"
            "   color: white;"
            "   border: none;"
        ...
        QPushButton *resetCvtButton = new QPushButton("Reset CVT");
        resetCvtButton->setStyleSheet(
            "QPushButton {"
            "   background-color: #505050;"
            "   color: white;"
            "   border: none;"
        ...
        // 连接信号
        QObject::connect(cvtButton, &QPushButton::clicked, [glWidget, levelLabel, paramTab]() {

            GLWidget* leftView = paramTab->property("leftGLWidget").value<GLWidget*>();
            GLWidget* rightView = paramTab->property("rightGLWidget").value<GLWidget*>();

            rightView->calculateCVT();
            leftView->setParameterizationTexCoords(rightView->paramTexCoords);

            levelLabel->setText(QString("Current Level: %1").arg(glWidget->currentCVTLevel));
        });
        
        QObject::connect(resetCvtButton, &QPushButton::clicked, [glWidget, levelLabel, paramTab]() {
            // 重置参数化状态
            GLWidget* leftView = paramTab->property("leftGLWidget").value<GLWidget*>();
            GLWidget* rightView = paramTab->property("rightGLWidget").value<GLWidget*>();
            ...
            rightView->currentCVTLevel = 0;
            levelLabel->setText("Current Level: 0");
        });
        
        cvtLayout->addRow("Iterations per Step:", cvtIterSpinBox);
        cvtLayout->addRow(levelLabel);
        cvtLayout->addRow(cvtButton);
        cvtLayout->addRow(resetCvtButton);
